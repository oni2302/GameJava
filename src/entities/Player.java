...
public class Player extends Entity {
	...
	private void initAttackBox() {
		attackBox = new Rectangle2D.Float(x, y, (int) (26 * Game.SCALE), (int) (20 * Game.SCALE));
		resetAttackBox();
	}
	...
	public void update() {
		...
		if (currentHealth <= 0) {
			if (state != DEAD) {
				state = DEAD;
                                playing.setStartTimes(System.currentTimeMillis());
                                playing.setDeadTimes(playing.getDeadTimes()+1);
				aniTick = 0;
				aniIndex = 0;
	...
	private void loadAnimations() {
		...
		for (int j = 0; j < animations.length; j++)
			for (int i = 0; i < animations[j].length; i++)
				animations[j][i] = img.getSubimage(i * 80, j * 80, 80, 80);
